package com.caiyu.lib_base.base;

import android.app.Application;
import android.os.Bundle;
import android.util.Log;

import androidx.annotation.NonNull;
import androidx.databinding.ObservableBoolean;
import androidx.databinding.ObservableField;
import androidx.lifecycle.AndroidViewModel;
import androidx.lifecycle.Lifecycle;
import androidx.lifecycle.LifecycleOwner;
import androidx.lifecycle.Observer;

import com.caiyu.lib_base.R;
import com.caiyu.lib_base.callback.BindingAction;
import com.caiyu.lib_base.callback.BindingCommand;
import com.trello.rxlifecycle3.LifecycleProvider;

import java.lang.ref.WeakReference;
import java.util.HashMap;
import java.util.Map;

import io.reactivex.disposables.CompositeDisposable;
import io.reactivex.disposables.Disposable;

/**
 * @author luys
 * @describe
 * @date 2019/4/17
 * @email samluys@foxmail.com
 */
public class BaseViewModel extends AndroidViewModel implements IBaseViewModel {
    private ViewLiveData mLiveData;
    //弱引用持有
    private WeakReference<LifecycleProvider> lifecycle;
    /**
     * 管理RxJava，主要针对RxJava异步操作造成的内存泄漏
     */
    private CompositeDisposable mCompositeDisposable;

    /**
     * 设置标题内容
     */
    public ObservableField<String> titleName = new ObservableField<>();
    /**
     * 设置标题内容字体颜色
     */
    public ObservableField<Integer> titleColor = new ObservableField<>(R.color.color_title);
    /**
     * 设置标题右侧文字内容
     */
    public ObservableField<String> rightName = new ObservableField<>();
    /**
     * 设置标题右侧Button内容
     */
    public ObservableField<String> rightButtonName = new ObservableField<>();
    /**
     * 设置标题右侧图片资源
     */
    public ObservableField<Integer> rightImage = new ObservableField<>();
    /**
     * 设置标题左侧图片资源
     */
    public ObservableField<Integer> leftImage = new ObservableField<>(R.drawable.ic_left_arrow);
    /**
     * 设置标题栏下方的分割线是否显示 默认显示
     */
    public ObservableBoolean divider = new ObservableBoolean(true);

    public BindingCommand backClick = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            clickBack();
        }
    });

    /**
     * 设置标题右侧点击事件
     */
    public BindingCommand rightClick = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            // 设置标题右侧点击事件
            clickRight();
        }
    });

    /**
     * 设置标题右侧点击事件
     */
    public BindingCommand right2Click = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            // 设置标题右侧点击事件
            click2Right();
        }
    });

    /**
     * 设置标题右侧点击事件
     */
    protected void clickRight() {

    }

    /**
     * 设置标题右侧点击事件
     */
    protected void click2Right() {

    }

    /**
     * 设置标题左侧点击事件
     */
    protected void clickBack() {
        finish();
    }

    public BaseViewModel(@NonNull Application application) {
        super(application);
    }

    /**
     * 注入RxLifecycle生命周期
     *
     * @param lifecycle
     */
    public void injectLifecycleProvider(LifecycleProvider lifecycle) {
        this.lifecycle = new WeakReference<>(lifecycle);
    }

    public LifecycleProvider getLifecycleProvider() {
        return lifecycle.get();
    }

    public ViewLiveData getLiveData() {
        if (mLiveData == null) {
            mLiveData = new ViewLiveData();
        }
        return mLiveData;
    }

    public void addSubscribe(Disposable disposable) {
        if (mCompositeDisposable == null) {
            mCompositeDisposable = new CompositeDisposable();
        }
        if (disposable == null) {
            return;
        }
        mCompositeDisposable.add(disposable);
    }

    public void showLoadingDialog() {
        showLoadingDialog("请稍后...");
    }

    public void showLoadingDialog(String title) {
        mLiveData.showDialogEvent.postValue(title);
    }

    public void dismissLoadingDialog() {
        mLiveData.dismissDialogEvent.call();
    }

    /**
     * 跳转页面
     *
     * @param clz 所跳转的目的Activity类
     */
    public void startActivity(Class<?> clz) {
        startActivity(clz, null);
    }

    /**
     * 跳转到Service
     *
     * @param clz    所跳转的目的Service类
     * @param bundle 跳转所携带的信息
     */
    public void startService(Class<?> clz, Bundle bundle) {

        Map<String, Object> params = new HashMap<>();
        params.put(ParameterField.CLASS, clz);
        if (bundle != null) {
            params.put(ParameterField.BUNDLE, bundle);
        }
        mLiveData.startServiceEvent.postValue(params);
    }

    /**
     * 跳转到私聊页面
     *
     * @param params  需要传递的参数
     */
    public void startPrivateChat(Map<String, String> params) {
        mLiveData.startPrivateChatEvent.postValue(params);
    }

    /**
     * 跳转页面
     *
     * @param clz    所跳转的目的Activity类
     * @param bundle 跳转所携带的信息
     */
    public void startActivity(Class<?> clz, Bundle bundle) {
        Map<String, Object> params = new HashMap<>();
        params.put(ParameterField.CLASS, clz);
        if (bundle != null) {
            params.put(ParameterField.BUNDLE, bundle);
        }
        mLiveData.startActivityEvent.postValue(params);
    }

    /**
     * 跳转容器页面
     *
     * @param canonicalName 规范名 : Fragment.class.getCanonicalName()
     */
    public void startContainerActivity(String canonicalName) {
        startContainerActivity(canonicalName, null);
    }

    /**
     * 跳转容器页面
     *
     * @param canonicalName 规范名 : Fragment.class.getCanonicalName()
     * @param bundle        跳转所携带的信息
     */
    public void startContainerActivity(String canonicalName, Bundle bundle) {
        Map<String, Object> params = new HashMap<>();
        params.put(ParameterField.CANONICAL_NAME, canonicalName);
        if (bundle != null) {
            params.put(ParameterField.BUNDLE, bundle);
        }
        mLiveData.startContainerActivityEvent.postValue(params);
    }

    /**
     * 跳转没有侧滑容器页面
     *
     * @param canonicalName 规范名 : Fragment.class.getCanonicalName()
     */
    public void startContainerNoSlideActivity(String canonicalName) {
        startContainerNoSlideActivity(canonicalName, null);
    }

    /**
     * 跳转没有侧滑容器页面
     *
     * @param canonicalName 规范名 : Fragment.class.getCanonicalName()
     * @param bundle        跳转所携带的信息
     */
    public void startContainerNoSlideActivity(String canonicalName, Bundle bundle) {
        Map<String, Object> params = new HashMap<>();
        params.put(ParameterField.CANONICAL_NAME, canonicalName);
        if (bundle != null) {
            params.put(ParameterField.BUNDLE, bundle);
        }
        mLiveData.startContainerNoSlideActivityEvent.postValue(params);
    }

    /**
     * 关闭界面
     */
    public void finish() {
        mLiveData.finishEvent.call();
    }



    public void showCommonDialog() {
        mLiveData.showCommonDialog.call();
    }

    public void showGiftDialog(String uid) {
        mLiveData.showGiftDialog.setValue(uid);
    }


    @Override
    public void onAny(LifecycleOwner owner, Lifecycle.Event event) {
    }

    @Override
    public void onCreate() {
    }

    @Override
    public void onDestroy() {

    }

    @Override
    public void onStart() {
    }

    @Override
    public void onStop() {
    }

    @Override
    public void onResume() {
    }

    @Override
    public void onPause() {
    }

    @Override
    public void registerRxBus() {
    }

    @Override
    public void removeRxBus() {
    }

    @Override
    protected void onCleared() {
        super.onCleared();
        //ViewModel销毁时会执行，同时取消所有异步任务
        if (mCompositeDisposable != null) {
            mCompositeDisposable.clear();
            Log.e("LUYS", "=====onCleared=====");
        }

    }

    public final class ViewLiveData extends SingleLiveEvent {
        private SingleLiveEvent<String> showDialogEvent;
        private SingleLiveEvent<Void> dismissDialogEvent;
        private SingleLiveEvent<Map<String, Object>> startActivityEvent;
        private SingleLiveEvent<Map<String, Object>> startContainerActivityEvent;
        private SingleLiveEvent<Map<String, Object>> startContainerNoSlideActivityEvent;
        private SingleLiveEvent<Void> finishEvent;
        private SingleLiveEvent<Void> onBackPressedEvent;
        private SingleLiveEvent<Void> showCommonDialog;
        private SingleLiveEvent<String> showGiftDialog;
        private SingleLiveEvent<Map<String, Object>> startServiceEvent;
        private SingleLiveEvent<Map<String, String>> startPrivateChatEvent;

        public SingleLiveEvent<Map<String, String>> getStartPrivateChatEvent() {
            return startPrivateChatEvent = createLiveData(startPrivateChatEvent);
        }



        public SingleLiveEvent<Void> getShowCommonDialog() {
            return showCommonDialog = createLiveData(showCommonDialog);
        }

        public SingleLiveEvent<String> getShowGiftDialog() {
            return showGiftDialog = createLiveData(showGiftDialog);
        }

        public SingleLiveEvent<String> getShowDialogEvent() {
            return showDialogEvent = createLiveData(showDialogEvent);
        }

        public SingleLiveEvent<Void> getDismissDialogEvent() {
            return dismissDialogEvent = createLiveData(dismissDialogEvent);
        }

        public SingleLiveEvent<Map<String, Object>> getStartActivityEvent() {
            return startActivityEvent = createLiveData(startActivityEvent);
        }

        public SingleLiveEvent<Map<String, Object>> getStartContainerActivityEvent() {
            return startContainerActivityEvent = createLiveData(startContainerActivityEvent);
        }

        public SingleLiveEvent<Map<String, Object>> getStartContainerNoSlideActivityEvent() {
            return startContainerNoSlideActivityEvent = createLiveData(startContainerNoSlideActivityEvent);
        }

        public SingleLiveEvent<Void> getFinishEvent() {
            return finishEvent = createLiveData(finishEvent);
        }

        public SingleLiveEvent<Void> getOnBackPressedEvent() {
            return onBackPressedEvent = createLiveData(onBackPressedEvent);
        }

        public SingleLiveEvent<Map<String, Object>> getStartServiceEvent() {
            return startServiceEvent = createLiveData(startServiceEvent);
        }

        private SingleLiveEvent createLiveData(SingleLiveEvent liveData) {
            if (liveData == null) {
                liveData = new SingleLiveEvent();
            }
            return liveData;
        }

        @Override
        public void observe(LifecycleOwner owner, Observer observer) {
            super.observe(owner, observer);
        }
    }

    public static final class ParameterField {
        public static String CLASS = "CLASS";
        public static String CANONICAL_NAME = "CANONICAL_NAME";
        public static String BUNDLE = "BUNDLE";
    }
}
