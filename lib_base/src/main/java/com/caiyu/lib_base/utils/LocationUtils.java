...
public class LocationUtils {
    ...
    public static void getCurrentLocation(Context context,LocationCallBack callBack){
        ...
        if (context == null){
            if (callBack != null) {
                callBack.onFail("请确保传入的参数context不为null");
            }
        }
        //如果系统版本号在23及其以上则检查权限
        if (Build.VERSION.SDK_INT >= 23 &&
                ContextCompat.checkSelfPermission(context, Manifest.permission_group.LOCATION) == PackageManager.PERMISSION_GRANTED){
            callBack.onFail("请确保已经获取定位权限");
        }
        //获取LocationManager对象
        locationM = (LocationManager) ApplicationUtils.getApp().getSystemService(Context.LOCATION_SERVICE);
        //实例化MyLocationListener
        locationListener = new MyLocationListener(locationM, callBack);
        //配置Criteria耗电低
        ...
        if (!TextUtils.isEmpty(bestProvider)){
            ...
            if (callBack != null) {
                callBack.onFail("无可用的定位方式，请打开GPS");
            }
        }
    }
